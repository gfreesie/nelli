...
- [~] **Deployed to `nelli.ssopros.com`** (DigitalOcean droplet `64.225.114.22` + Plesk; docroot `/var/www/vhosts/ssopros.com/site2`). Static `dist/` shipped via scp+tar; OG/social URLs repointed to the live domain (no Vite `base` change — served at the subdomain root). **Live over HTTP only for now** — HTTPS is blocked by an invalid Plesk license ("allows 0 sites"); the HTTP→HTTPS redirect is disabled (`-ssl-redirect false`) so the site works. To finish: reactivate the Plesk license, then `plesk bin extension --exec letsencrypt cli.php -d nelli.ssopros.com -m garryalc@gmail.com` and flip `-ssl-redirect true`.
